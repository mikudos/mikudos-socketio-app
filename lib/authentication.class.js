...
class Authentication {
    constructor({ protocol, host, port, path, method, headers } = {
        protocol: 'http',
        host: '127.0.0.1',
        port: 80,
        path: '/authentication',
        method: 'POST',
        headers: {}
    }, { tokenPath = 'accessToken', eventPath = 'authentication', authJoinCallback = async (socket, app) => { } } = {}) {
        this.requsetOption = {
            method: 'POST',
            headers: {
    ...
    register(app, socket) {
        socket.on(this.eventPath, async (data, callback) => {
            ...
            let userId = socket.mikudos.user[app.get('authentication.entityId') || 'id'];
            if (userId) {
                socket.join(userId);
            }
