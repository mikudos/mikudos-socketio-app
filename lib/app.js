...
class Application {
    constructor(rootIo, rootNamespace) {
        this.rootIo = rootIo;
        this.rootNamespace = rootNamespace;
        rootIo.adapter(socket_io_redis_1.default({ host: 'localhost', port: 6379 }));
        this.io = rootNamespace ? rootIo.of(rootNamespace) : rootIo.of('/');
        this.settings = lodash_1.default.merge({}, config_1.default);
    }
    bind(io, rootNamespace) {
        this.rootIo = io;
        io.adapter(socket_io_redis_1.default({ host: 'localhost', port: 6379 }));
                ...
                socket.on(this.authentication.eventPath, async (data, callback) => {
                    ...
                    socket.join('authenticated', () => {
                        this.authentication &&
                            ...
                            this.authentication.authJoinCallback(socket, this.io);
                    });
                });
            }
